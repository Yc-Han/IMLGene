...
We may notice a peak at the second half of the absolute sum curve. We speculate this is due to the loess method applied, and the effect is more random than artifact. The totally unpatterned sum plot justifies this.

This can also be the effect of a LSTM model. Can be further studied.

If the peak is not random and the loess is correct, moving the targeted sequence should also change the peak. As a mere check we move our instance 250 bp to the right, that is 748 to 1348 bp. The smooth curve looks the same as now, for simplicity we do not show it again.
...
We further studied wobleness (see Durrant & Bhatt). In this case, more randomness is introduced by the fact that we only calculated IG once. In RSDexport, the average importance almost consistently follow the 2-1-3 structure. Here, we tested some other instances and baselines and wobleness is not always detected.
...
print("ACGT content of selected, consecutive features:")
table(consecutive_trip)
print("Table of corresponding amino acids of selected, consecutive features:")
...
We also tried adversarial instances. We replace the selected spots of the instance all with C, and the confidence for 16S rRNA drops from **0.906 to 0.176**! Also replacing them with T reduces the confidence to 0.20. Replacing with A and G is not effective.

We compare them with an exact same number of randomly selected positions and substitute them accordingly too.

We also tried replacing them with 0.25, and the confidence drops only to 0.7. There is also only 0.04 difference of confidence between the selected features and randomly sampled features. 
