...
hm2 <- ggplot(pcss_final, aes(x = cod1, y = cod23, fill = value)) +
  geom_tile() +
  geom_text(aes(label = key), color = "black", size = 3) +
  scale_fill_gradient2(low = "blue", mid = "lightgrey", high = "red", midpoint = 0.7) + 
  theme_bw() +
...
result_df$significance <- ifelse(result_df$p_value < 0.05 / 600, "*", "")
```
...
ggplot(result_df, aes(x = position)) +
  ...
  labs(x = "Locus Index", y = "Mean Locus Abs Sum", subtitle = "Sample Size = 50, Selected 33 out of 600, Bonferroni Correction")
```
